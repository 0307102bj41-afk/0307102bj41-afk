...
**핵심 트러블슈팅** — 조명·색상 과의존 오분류(Grayscale 전처리), 과적합(EarlyStopping·Dropout), 데이터 편향(Grad-CAM 기반 데이터 보강), TF 버전 추론 불일치(Linux 환경 일원화)
...
**핵심 트러블슈팅** — ADC 변환 미완료 쓰레기 값(EOC 폴링), I2C LCD 초기화 타이밍 깨짐, UART 수신 무한 대기·패킷 깨짐(타임아웃 + 상태머신 파서)
...
<img src="https://0307102bj41-afk.github.io/assets/racing.gif" width="80%" alt="RC카 주행 장면">
<br/>
...
**핵심 트러블슈팅** — 택타임 레지스터 누적 버그(공정 시작 초기화), 선택/전체 비움 위치 충돌(B접점 인터락), 층별 절대 위치 비효율(AXIS Pointer INC 상대 제어 전환)
